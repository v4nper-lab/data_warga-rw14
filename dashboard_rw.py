    ...
    if not st.session_state["cari_terbuka"]:
        ...
        if kata_kunci:
            ...
            if kolom_nama_warga and kolom_kk:
                ...
                # Saring secara ketat: baris wajib berstatus Kepala Keluarga DAN namanya mengandung kata kunci utuh (misal "AAN")
                ...
                # Jika kosong, cari yang namanya persis mengandung kata tersebut
                ...
                if len(nomor_kk_ditemukan) > 0:
                    for no_kk_val in nomor_kk_ditemukan:
                        # Ambil HANYA anggota keluarga yang memiliki Nomor KK persis sama
                        ...
                        # Tampilkan Tabel Anggota Keluarga secara lengkap
                        st.dataframe(hasil_keluarga, use_container_width=True, hide_index=True)
                        st.markdown("<hr style='margin: 30px 0;'>", unsafe_allow_html=True)
                else:
                    st.warning(f"⚠️ Tidak ditemukan data Kartu Keluarga untuk pencarian Kepala Keluarga dengan nama '{kata_kunci}'.")
            else:
