...
with tab5:
    ...
    if not st.session_state["cari_terbuka"]:
        ...
        # Pilihan mode pencarian: Berdasarkan Nama Kepala Keluarga atau Berdasarkan Nomor KK langsung
        metode_cari = st.radio("Pilih Metode Pencarian:", ["Berdasarkan Nama Kepala Keluarga", "Berdasarkan Nomor Kartu Keluarga (No. KK)"], horizontal=True)
        ...
        if metode_cari == "Berdasarkan Nama Kepala Keluarga":
            kata_kunci = st.text_input("🔎 Masukkan Nama Kepala Keluarga (Misal: AAN TONI FAUYI atau AAN):")
            if kata_kunci and kolom_nama_warga and kolom_kk:
                kw_clean = str(kata_kunci).strip().upper()
                
                # Saring baris yang mengandung nama kepala keluarga secara persis
                df_kk_filter = pd.DataFrame()
                if kolom_hub:
                    df_kk_filter = df[
                        df[kolom_hub].astype(str).str.upper().str.contains("KEPALA|KDH", regex=True, na=False) &
                        df[kolom_nama_warga].astype(str).str.strip().str.upper().str.contains(kw_clean)
                    ]
                ...
                if len(nomor_kk_ditemukan) > 0:
                    ...
                    st.warning(f"⚠️ Tidak ditemukan data Kartu Keluarga untuk nama '{kata_kunci}'.")
        else:
            no_kk_input = st.text_input("🔎 Masukkan Nomor Kartu Keluarga (No. KK):")
            if no_kk_input and kolom_kk:
                no_kk_clean = str(no_kk_input).strip()
                hasil_keluarga = df[df[kolom_kk].astype(str).str.strip() == no_kk_clean]
                
                if not hasil_keluarga.empty:
                    kepala_keluarga_row = hasil_keluarga[
                        hasil_keluarga[kolom_hub].astype(str).str.upper().str.contains("KEPALA|KDH", regex=True, na=False)
                    ] if kolom_hub else pd.DataFrame()
                    ...
                    <div style="background-color: #ffffff; padding: 20px; border-radius: 12px; border: 2px solid #0D47A1; margin-bottom: 25px; box-shadow: 0px 4px 10px rgba(0,0,0,0.08);">
                        <div style="text-align: center; border-bottom: 2px solid #0D47A1; padding-bottom: 10px; margin-bottom: 15px;">
                            <h3 style="margin: 0; color: #0D47A1; font-size: 20px;">KARTU KELUARGA (KK)</h3>
                            <p style="margin: 3px 0 0 0; font-weight: bold; color: #555; font-size: 14px;">No. KK : {no_kk_clean}</p>
                        </div>
                        <div style="display: flex; justify-content: space-between; font-size: 14px; margin-bottom: 15px; color: #333;">
                            <div>
                                <p style="margin: 4px 0;"><b>Nama Kepala Keluarga :</b> {nama_kk}</p>
                    ...
                    st.warning(f"⚠️ Tidak ditemukan Kartu Keluarga dengan nomor '{no_kk_clean}'.")
